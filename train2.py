...
def train(model, attack, device, train_loader, optimizer, epoch):
    model.train()
    correct = 0
...
if __name__ =='__main__':
    ...
    model =   models.resnet18().to(device) #CNNmodel.Net().to(device)
    optimizer = optim.SGD(model.parameters(), lr=0.0001, momentum=0.5)
    attack = torchattacks.PGD(model, eps=EPS, alpha=ALPHA, steps=STEPS)
    save_model = True
    for epoch in range(1, 100 + 1):     ## 5 batches
        print(epoch)
        train(model, attack, device, train_loader, optimizer, epoch)
        test(model, device, test_loader)

        if (save_model):
            torch.save(model.state_dict(), "./checkpoint/cifar10_pgdtraining.pt")